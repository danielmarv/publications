...
import { deleteFile, draftDocument, renameFile, undraftDocument, updateFileUsers } from "@/lib/actions/file.actions"
...
const ActionDropdown = ({ file }: { file: Models.Document }) => {
  ...
  const [Description, setDescription] = useState("") // Updated initial value
    ...
    return (
      <DialogContent className="shad-dialog button">
        <DialogHeader className="flex flex-col gap-3">
          ...
          {value === "draft" && (
            <p>Are you sure you want to draft the document titled &quot;{file.name}&quot; for publishing?</p>
          )}
          {value === "undraft" && <p>Are you sure you want to undraft the document titled "{file.name}" ?</p>}
        </DialogHeader>
          ...
          {actionsDropdownItems.map((actionItem) => (
            <DropdownMenuItem
              ...
              onClick={() => {
                ...
                if (
                  ["publish", "rename", "share", "delete", "details", "draft", "undraft"].includes(actionItem.value)
                ) {
                  setIsModalOpen(true)
                }
              }}
            >
              {actionItem.value === "download" ? (
                ...
                >
                  <Image src={actionItem.icon || "/placeholder.svg"} alt={actionItem.label} width={30} height={30} />
                  {actionItem.label}
                </Link>
              ) : (
